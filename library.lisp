...
mkdir /dev/shm/claw-cxx-raylib/fasl/lib
     ...
     (let ((arch "x86_64-pc-linux-gnu")
	   ...
	   (cflags (format nil "-O0 -g3 -I~A" cl-user::*claw-raylib-inc*))
	 ...
	 (let* ((source (namestring (merge-pathnames (format nil "lib~A-adapter.~A.c" lib arch) path)))
		...
		(obj (namestring (merge-pathnames (format nil "lib~A-adapter.o" lib) destdir)))
		(dll (namestring (merge-pathnames (format nil "lib~A-adapter.so" lib) destdir)))
